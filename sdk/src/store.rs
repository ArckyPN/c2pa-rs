...
impl Store {
    ...
    fn start_save_bmff_fragmented(
        ...
        match window_size {
            Some(window) => {
                // Merkle Tree signing
                let (sign_fragments, local_id, unique_id) = match window {
                    0 => {
                    ...
                    _ => {
                        ...
                        let mut groups = fragments.chunks(window);
            ...
            None => {
                ...
                if fragments.len() != 1 {
                    return Err(Error::BadParam(
                        "RollingHash signing requires exactly one fragment".to_string(),
                    ));
                }
                let fragment = &fragments[0];

                bmff_hash.shift_rolling_hash();

                bmff_hash.add_rolling_hash_fragment(pc.alg(), &asset_path, fragment, output_dir)?;
            }
        }
